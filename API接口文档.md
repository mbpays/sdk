...
查询订单的详细信息，包括订单状态、金额、时间等信息。

#### 接口地址

```
POST /merchant/orderinfo
```
...
```bash
curl -X POST "https://www.mbpay.world/merchant/orderinfo" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "order_no=ORD202501011200001234567890" \
  -d "merchant_id=12345" \
  -d "app_id=your_app_id_123" \
...
| platform_fee | int64 | 平台手续费（MB，最小单位：100 = 1MB） |
| status | int | 订单状态（0:待支付, 1:已支付, 2:已取消, 3:已过期） |
| status_text | string | 订单状态文本描述 |
| expires_at | string | 订单过期时间（格式：YYYY-MM-DD HH:mm:ss） |
| created_at | string | 订单创建时间（格式：YYYY-MM-DD HH:mm:ss） |
| paid_at | string | 订单支付时间（格式：YYYY-MM-DD HH:mm:ss，未支付时为空） |
...
| 12013 | 订单号为空 | 检查请求参数，确保 order_no 已传递 |
| 12014 | 订单不存在 | 检查订单号是否正确，或订单是否属于当前商户 |
